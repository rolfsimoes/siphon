...
- Added `requireNamespace()` checks for `mirai` and `future` backends with clear error messages.
- Added async-backend scheduling tests for `mirai` and `future` backends.
